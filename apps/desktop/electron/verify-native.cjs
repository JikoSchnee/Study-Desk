...
function verifyBuildOutputs() {
  ...
  const sourceModule = join(workspaceRoot, "node_modules", "better-sqlite3");
  const standaloneModule = join(desktopRoot, ".next", "standalone", "node_modules", "better-sqlite3");
  verifyNativeAddon(process.execPath, sourceModule, { cwd: workspaceRoot, runtimeName: `Node ${process.versions.node} (ABI ${process.versions.modules})` });
  verifyNativeAddon(require("electron"), standaloneModule, { cwd: workspaceRoot, runtimeName: `Electron ${require("electron/package.json").version}` });
}
