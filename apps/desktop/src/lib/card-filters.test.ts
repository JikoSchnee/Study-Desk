...
const learning: Record<string, CardLearningSummary> = {
  a: { cardId: "a", initialStudyAt: "2026-01-01T00:00:00.000Z", nextReviewAt: "2026-01-10T00:00:00.000Z", lastReviewAt: "2026-01-09T00:00:00.000Z", practiceCount: 2, reviewCount: 1, hasInitialPractice: true, averageScore: 88, fsrsDifficulty: 6.4, fsrsStability: 12 },
  b: { cardId: "b", initialStudyAt: "2026-01-01T00:00:00.000Z", nextReviewAt: "2026-01-08T00:00:00.000Z", lastReviewAt: "2026-01-10T00:00:00.000Z", practiceCount: 1, reviewCount: 0, hasInitialPractice: true, averageScore: null, fsrsDifficulty: 2.8, fsrsStability: 2 },
};

describe("card filters", () => {
  it("maps FSRS difficulty boundaries into four visible tiers", () => {
    expect(difficultyTier(1)?.label).toBe("Easy");
    expect(difficultyTier(3)?.label).toBe("Normal");
    expect(difficultyTier(6)?.label).toBe("Hard");
    expect(difficultyTier(8)?.label).toBe("Very Hard");
    expect(difficultyTier(10)?.label).toBe("Very Hard");
