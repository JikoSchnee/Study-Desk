...
const tables = ["cards", "card_relations", "review_state", "review_logs", "initial_study_logs", "daily_plans", "daily_tasks", "interview_sessions", "interview_turns", "knowledge_maintenance_proposals", "knowledge_sync_records", "practice_focus", "settings", "tags"] as const;
...
export function parseBackup(value: unknown): AppBackup {
  ...
  if ((backup.version !== 1 && backup.version !== 2 && backup.version !== 3 && backup.version !== 4 && backup.version !== BACKUP_VERSION) || !backup.tables || typeof backup.tables !== "object") throw new Error("不支持此备份版本。");
  const legacyTables = tables.filter((table) => table !== "card_relations" && table !== "initial_study_logs" && table !== "tags");
