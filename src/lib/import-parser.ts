...
import { answerPointsFromText, emptyMapping, previewImport, questionVariantsFromText, splitTags, type ImportColumnMapping, type ImportPreviewRow } from "@/lib/import";
...
async function workbookFromFile(file: File) {
  validateUpload(file);
  return XLSX.read(Buffer.from(await file.arrayBuffer()), { type: "buffer", cellFormula: false, cellHTML: false, cellText: true });
}
...
export async function previewWorkbook(file: File, sheetName: string, mapping: ImportColumnMapping, existingQuestions: string[]) {
  const workbook = await workbookFromFile(file);
  ...
  const positions = { question: indexFor("question"), variants: indexFor("variants"), answer: indexFor("answer"), hint: indexFor("hint"), track: indexFor("track"), tags: indexFor("tags"), difficulty: indexFor("difficulty") };
  const raw = rows.slice(1, 501).map((row, index) => {
    ...
    return {
      ...
      hint, rowNumber: index + 2, hasContent: Boolean(question || variants || answer || hint || rawTrack || rawTags || rawDifficulty),
    };
  }).filter((row) => row.hasContent);
  const dedupe = previewImport(raw.map((row) => ({ ...row, questionVariants: questionVariantsFromText(row.variants) })), existingQuestions);
  ...
  const preview: ImportPreviewRow[] = raw.map((row) => {
    const key = `${row.question}\u0000${row.answer}`;
    const reason = rejected.get(row.question.trim().toLowerCase());
    const status = accepted.has(key) ? "valid" : reason?.includes("已存在") ? "duplicate" : "invalid";
    const hintCount = row.hint.split(/\r?\n/).filter((item) => item.trim()).length;
    const answerPoints = answerPointsFromText(row.answer, row.hint);
    const note = hintCount > answerPoints.length ? "提示多于答案要点，额外提示将忽略。" : undefined;
    return { id: randomUUID(), rowNumber: row.rowNumber, status, reason, note, card: { question: row.question, questionVariants: questionVariantsFromText(row.variants), answerPoints, track: row.track, tags: row.tags, difficulty: row.difficulty } };
  });
