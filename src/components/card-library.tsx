...
import { Archive, ArrowUpDown, BookOpenText, CalendarClock, CheckCircle2, Clock3, Download, FilePlus2, LibraryBig, MessageSquareText, PencilLine, Search, Sparkles, Tag, Trash2, Undo2, X } from "lucide-react";
...
export function CardLibrary() {
  ...
  const exportSelected = (format: "json" | "csv") => {
    ...
    const content = format === "json" ? JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), cards: selected }, null, 2) : ["question,answer,track,tags,status", ...selected.map((card) => [card.question, card.answer, card.track, card.tags.join("|"), card.status].map((value) => `\"${value.replaceAll("\"", "\"\"")}\"`).join(","))].join("\n");
