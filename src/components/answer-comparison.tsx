...
export function AnswerComparisonView({ comparison, answer }: { comparison: AnswerComparison; answer: string }) {
  ...
  return <section className="answer-comparison" aria-label="参考答案与本次回答对照">
    ...
    <div className="comparison-columns">
      <section className="comparison-reference" aria-label="原始参考答案"><p className="eyebrow">原始答案结构</p><div className="comparison-sections">{grouped.map((group) => <section className={`comparison-section ${group.role}`} key={group.role}><div><strong>{sectionLabels[group.role]}</strong>{group.role !== "key" && <small>结构项</small>}</div><ol>{group.points.map(({ point, index }) => <li key={point.answerPointId} className={`${point.status}${activePoint === index ? " linked-active" : activePoint !== null ? " linked-muted" : ""}`} tabIndex={0} onMouseEnter={() => setActivePoint(index)} onMouseLeave={() => setActivePoint(null)} onFocus={() => setActivePoint(index)} onBlur={() => setActivePoint(null)}><span className="comparison-index">{index + 1}</span><div><strong>{labels[point.status]}</strong><p>{point.reference}</p></div></li>)}</ol></section>)}</div></section>
