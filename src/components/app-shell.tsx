...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-frame">
    ...
    <nav className="bottom-nav" aria-label="移动端主导航">{nav.slice(0, 5).map(([href, label, Icon]) => <Link key={href} href={href} className={pathname === href ? "active" : ""}><Icon size={20}/><span>{label}</span></Link>)}<button type="button" className={moreOpen || pathname === "/knowledge-base" || pathname === "/settings" ? "active" : ""} onClick={() => setMoreOpen(true)}><Menu size={20}/><span>更多</span></button></nav>
