...
import { CheckCircle2, ChevronLeft, CircleAlert, FileSpreadsheet, UploadCloud, X } from "lucide-react";
...
export function CardWorkspace({ initialMode, onClose, onComplete }: CardWorkspaceProps) {
  ...
  const saveCard = useCallback(async (): Promise<string | void> => {
    ...
    try {
      ...
      setDraft(freshDraft()); await load();
      onComplete?.("卡片已加入首次学习队列；先看懂答案要点，明天再开始第一次主动回忆。"); onClose?.();
    } catch {
