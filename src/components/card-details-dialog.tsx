...
function answerPointRole(point: AnswerPoint): AnswerPointRole {
  return point.role === "opening" || point.role === "closing" ? point.role : "key";
}
...
type ChartPoint = { label: string; fullDate: string; score: number; interval: number | null; rating: RatingName; isInitial: boolean };
...
export function CardDetailsDialog({ card, relatedCards, learning, onClose }: { card: Card; relatedCards: Array<Card & { relationType: CardRelationType }>; learning: CardLearningDetails; onClose: () => void }) {
  ...
  const scoreData: ChartPoint[] = learning.history.filter((item) => item.score !== null).map((item) => ({
  ...
  const difficulty = learning.fsrsDifficulty === null ? learning.initialStudyAt ? "等待首次回忆" : "待首次学习" : `${tier?.label ?? "—"} · ${learning.fsrsDifficulty.toFixed(1)} / 10`;
