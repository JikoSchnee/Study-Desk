...
export default function SettingsPage() {
  ...
  const checkForUpdates = async () => {
    ...
    try { setUpdateResult(await window.mockInterviewDesktop.updates.check() as UpdateCheckResult); setLastCheckedAt(new Date()); }
  ...
  return <PageLayout className="settings-page">
    ...
    <section className="settings-utility-grid" aria-label="辅助功能">
      ...
      <Panel className="desktop-updater"><div className="desktop-updater-heading"><div><p className="eyebrow"><Download size={15}/> 桌面应用</p><h2>手动更新</h2></div>{isDesktop && <Button type="button" variant="outline" disabled={checkingUpdate} onClick={() => void checkForUpdates()}><RefreshCw size={16}/>{checkingUpdate ? "正在检查…" : "检查更新"}</Button>}</div>{!isDesktop && <p className="muted-copy">请在桌面应用中检查更新。</p>}{updateResult?.state === "error" && <p className="desktop-update-error" role="status">{updateResult.message}</p>}{updateResult?.state === "current" && <p className="desktop-update-ok" role="status">已是最新版本：v{updateResult.currentVersion}{lastCheckedAt ? ` · 最后检查时间：${lastCheckedAt.toLocaleString()}` : ""}</p>}{updateResult?.state === "available" && <div className="desktop-update-card" role="status"><strong>有新版本：v{updateResult.latestVersion}</strong><a className="button" href={updateResult.url} target="_blank" rel="noreferrer"><ExternalLink size={16}/> 请前往下载页下载</a></div>}</Panel>
    </section>
