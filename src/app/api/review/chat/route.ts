...
import { generateLearningChatCardDraft, generateLearningChatResponse, hasRemoteModelConfig, type LearningChatMessage } from "@/lib/ai";
...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "chat") return NextResponse.json({ message: await generateLearningChatResponse(card, messages, input.message) });
