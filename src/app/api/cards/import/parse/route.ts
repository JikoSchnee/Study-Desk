...
const mappingSchema = z.object({ question: z.string(), variants: z.string().default(""), answer: z.string(), hint: z.string(), track: z.string(), tags: z.string(), difficulty: z.string() });
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await previewWorkbook(file, sheetName, mapping, listCards().flatMap(allQuestionTexts)));
  } catch (error) {
