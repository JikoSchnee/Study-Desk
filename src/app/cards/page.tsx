...
import { AnswerStructureEditor as AnswerPointsEditor, QuestionVariantsEditor } from "@/components/card-form-editors";
...
const point = (content = "", hint = "", note = ""): AnswerPoint => ({ id: pointId(), content, hint, note });
const freshDraft = (): CardDraft => ({ question: "", questionVariants: [], answerPoints: [point()], note: "", track: "Agent", tags: "", source: "" });

function parseAdvanced(text: string) {
  if (/^question,answer/i.test(text.trim())) return text.trim().split(/\r?\n/).slice(1).map((line) => { const [question, answer, track = "Agent", tags = ""] = line.split(","); return { question, answer, track, tags: tags.split("|").filter(Boolean) }; });
  return text.split(/^##\s+/m).filter(Boolean).map((part) => { const [question, ...body] = part.split("\n"); return { question: question.trim(), answer: body.join("\n").trim(), track: "Agent", tags: [] }; });
}
...
export default function CardsPage() {
  ...
  const importAdvanced = async () => { const rows = parseAdvanced(advanced); if (!rows.length) return; const response = await fetch("/api/cards/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cards: rows }) }); const data = await response.json(); setNotice(`导入 ${data.accepted.length} 张卡片${data.rejected.length ? `，跳过 ${data.rejected.length} 条重复或无效内容` : ""}。`); setAdvanced(""); load(); };
  ...
  useEffect(() => {
    ...
    setMode("manual");
    setDraft({ question: tutorialCard.question, questionVariants: tutorialCard.questionVariants.map((item) => ({ ...item })), answerPoints: tutorialCard.answerPoints.map((item) => ({ ...item })), note: tutorialCard.note, track: "演示", tags: tutorialCard.tags.join(", "), source: tutorialCard.source ?? "" });
  ...
  const saveCard = useCallback(async (): Promise<string | void> => {
    ...
    try {
      ...
      if (!response.ok) { const message = data.error ?? "保存失败，请检查问题和答案要点。"; setNotice(message); return message; }
      if (tutorialSave) { setNotice("演示卡已保存，接下来去完成首次练习。 "); completeCheckpoint("tutorial-card-saved"); await load(); return; }
      setNotice("卡片已加入待首次练习；完成第一次真实作答后，才会进入复习节奏。");
      completeCheckpoint("card-created"); setDraft(freshDraft()); load();
    } catch { const message = "保存失败，网络连接可能已中断。请重试。"; setNotice(message); return message; }
  ...
  return <PageLayout className="cards-page"><LLMConfigurationDialog open={needsLLMConfiguration} onClose={() => setNeedsLLMConfiguration(false)} purpose="AI 补充问法" /><div className="cards-workspace"><PageHeader eyebrow={<><FileUp size={15}/> 个人题库</>} title="把资料，变成可回答的问题。" description="一张卡聚焦一个知识点；多种问法共享清晰答案，练的是理解，不是题面。" tour="cards" actions={<div className="cards-import-actions"><Button onClick={openImport}><FileSpreadsheet size={18}/> 从文件导入</Button><details className="template-download"><summary>下载模板文件</summary><div className="template-download-options"><p>CSV 模板中，其他问法、答案要点和回忆提示均可在同一单元格内换行；答案与提示会按行配对。</p><a href="/cards-import-template.md" download>Markdown 模板</a><a href="/cards-import-template.csv" download>CSV 模板（完整字段）</a></div></details></div>} />{notice && <div className="notice" role="status" style={{ marginBottom: 20 }}>{notice}</div>}
    ...
    <details className="advanced-import"><summary>高级导入：粘贴 Markdown 或 CSV</summary><p>适合迁移旧笔记；普通导入请优先使用上方的可视化工作台。</p><textarea aria-label="高级导入内容" value={advanced} onChange={(event) => setAdvanced(event.target.value)} placeholder={"## 什么是 Agent？\n…你的答案…"}/><div className="form-actions"><Button variant="ghost" onClick={importAdvanced}>导入粘贴内容</Button></div></details></div></PageLayout>;
}
