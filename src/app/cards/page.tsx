...
export default function CardsPage() {
  ...
  const scrollToPageTopAfterRender = useCallback(() => {
  ...
  const saveCard = useCallback(async (): Promise<string | void> => {
    ...
    try {
      ...
      if (tutorialSave) { setNotice("演示卡已保存，接下来去完成首次学习。 "); completeCheckpoint("tutorial-card-saved"); await load(); scrollToPageTopAfterRender(); return; }
      completeCheckpoint("card-created"); setDraft(freshDraft()); void load().finally(scrollToPageTopAfterRender);
  ...
  }, [activeId, completeCheckpoint, draft, load, scrollToPageTopAfterRender, tutorialCardId]);
