...
export default function ReviewPage() {
  ...
  const actOnFeedback = async (action: "weak" | "priority" | "removeWeak") => {
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      else setSubmitError("未收到评估结果，请重试。");
    } catch { setSubmitError("提交失败，答案已保留。请检查网络后重试。"); }
    finally { setBusy(false); }
  };

  const confirm = async (rating: RatingName) => {
    if (!card || !session || session === "random") return;
    if (session === "weak") {
    ...
    try {
      ...
      if (!result.error) { window.localStorage.removeItem(`mock-interview:draft:${session}:${card.id}`); await loadSession(session); }
    } finally { setBusy(false); }
  };

  if (!progress && session === null) return <div className="loading">正在准备今天的练习…</div>;

  if (session === null && progress) return <>
    <header className="page-header review-choice-header">
      <div><p className="eyebrow"><BrainCircuit size={15}/> 主动回忆</p><h1>今天想先练哪一类？</h1><p>导入不会算作练习；第一次作答与之后的复习，会分别留下清晰记录。</p></div>
      <Button variant="secondary" onClick={loadRandom}><Dices size={17}/> 随机抽题</Button>
    </header>
    <section className="review-queue-grid" aria-label="选择练习队列">
      {(["initial", "review", "weak"] as const).map((kind) => {
        const copy = queueCopy[kind];
        const Icon = copy.icon;
        const stats = progress[kind];
        return <article className={`review-queue-card ${kind}`} key={kind}>
  ...
  if (!card && session === "random") return <><header className="page-header"><div><p className="eyebrow"><Dices size={15}/> 随机练习</p><h1>题库还没有可抽取的卡片。</h1><p>先创建一张卡片，把一个知识点练成能说出口的话。</p></div><Button variant="secondary" onClick={leaveSession}>返回练习选择</Button></header><EmptyState title="还没有可练习的卡片" detail="创建卡片后就可以随时随机抽题。" action={<Link href="/cards"><Button>去建立卡片</Button></Link>} /></>;
  ...
  if (!card && session !== null && session !== "random") {
    ...
    return <><header className="page-header"><div><p className="eyebrow"><BookOpenCheck size={15}/> {copy.label}</p><h1>{copy.label}已完成。</h1><p>{progress ? `${progressText(progress[session])}。` : ""}{other && otherStats.pending ? ` 接下来还有 ${otherStats.pending} 道${queueCopy[other].label}。` : " 今天的练习告一段落。"}</p></div><Button variant="secondary" onClick={leaveSession}>返回练习选择</Button></header><EmptyState title={other && otherStats.pending ? "继续下一类练习" : "今天的练习告一段落"} detail={other && otherStats.pending ? `确认后进入${queueCopy[other].label}，进度会继续累计。` : "明天再来复习，记忆会更牢。"} action={other && otherStats.pending ? <Button onClick={() => startQueue(other)}>继续{queueCopy[other].label}<ArrowRight size={17}/></Button> : <Link href="/cards"><Button>查看题库</Button></Link>} /></>;
  }
  ...
  return <>
    ...
    <header className="page-header">
      <div><p className="eyebrow"><BrainCircuit size={15}/> {isRandom ? "随机练习" : sessionCopy?.label}</p><h1>{isRandom ? "随机抽一题，说说看。" : isInitial ? "先想，再说出来。" : queue === "weak" ? "把难点练扎实。" : "把记忆再叫回来。"}</h1><p>{isRandom ? "这次练习只提供反馈，不会影响复习排程。" : isInitial ? "这是第一次真实作答；确认记忆状态后，才会开始进入复习节奏。" : queue === "weak" ? "薄弱复习不改变排程，练好后可以把它移出队列。" : "说不完整没关系，关键是把思路调出来。"}</p></div>
      <div className="header-actions">{isRandom ? <Chip tone="blue">随机练习</Chip> : stats && <Chip tone={isInitial ? "green" : "blue"}>{progressText(stats)}</Chip>}<Button variant="secondary" onClick={isRandom ? loadRandom : leaveSession}>{isRandom ? <><Dices size={17}/> 再抽一题</> : "切换练习"}</Button></div>
    </header>
    <article className="review-card">
      ...
      </div> : isRandom ? <div className="random-feedback"><div className="feedback"><strong>{evaluation.score} 分 · 随机练习反馈</strong><p>{evaluation.feedback}</p>{evaluation.gaps.length > 0 && <p>待补充：{evaluation.gaps.join("、")}</p>}</div><AnswerComparisonView comparison={evaluation.comparison} answer={answer}/><MoreQuestions/><Button variant="secondary" onClick={loadRandom}><Dices size={17}/> 再抽一题</Button></div> : <div className="stack"><div className="feedback"><strong>{evaluation.score} 分 · 建议：{evaluation.suggestedRating}</strong><p>{evaluation.feedback}</p>{evaluation.gaps.length > 0 && <p>待补充：{evaluation.gaps.join("、")}</p>}</div><AnswerComparisonView comparison={evaluation.comparison} answer={answer}/><div className="feedback-actions"><p className="eyebrow">把反馈变成下一步</p><Button variant="secondary" onClick={() => actOnFeedback("weak")}>加入薄弱复习</Button><Button variant="outline" onClick={() => actOnFeedback("priority")}>下次优先练习</Button><Button variant="ghost" onClick={createSupplement}>生成补充卡</Button><Button variant="ghost" disabled={!llmConfigured || followUpBusy || Boolean(followUpQuestion)} onClick={generateFollowUp}><Sparkles size={16}/> {followUpBusy ? "正在生成追问…" : followUpQuestion ? "已生成追问" : "AI 拓展追问"}</Button>{queue === "weak" && <Button variant="ghost" onClick={() => actOnFeedback("removeWeak")}>移出薄弱队列</Button>}{feedbackNotice && <span className="muted-copy" role="status">{feedbackNotice}</span>}</div>{followUpQuestion && <section className="follow-up-card"><p className="eyebrow"><Sparkles size={15}/> AI 拓展追问</p><h3>{followUpQuestion}</h3>{!followUpFeedback ? <><textarea className="answer compact-answer" value={followUpAnswer} onChange={(event) => setFollowUpAnswer(event.target.value)} placeholder="回答这条追问；不会改变本卡的 FSRS 排程。" /><Button disabled={!followUpAnswer.trim() || followUpBusy} onClick={evaluateFollowUp}>{followUpBusy ? "正在评估…" : "提交追问回答"}</Button></> : <div className="feedback"><strong>{followUpFeedback.score} 分 · 追问反馈</strong><p>{followUpFeedback.feedback}</p>{followUpFeedback.gaps.length > 0 && <p>待补充：{followUpFeedback.gaps.join("、")}</p>}</div>}</section>}<MoreQuestions/><div><p className="eyebrow">确认本次记忆状态</p><div className="rating-row"><Button className="again" disabled={busy} onClick={() => confirm("again")}>忘记</Button><Button className="hard" disabled={busy} onClick={() => confirm("hard")}>困难</Button><Button className="good" disabled={busy} onClick={() => confirm("good")}>良好</Button><Button className="easy" disabled={busy} onClick={() => confirm("easy")}>轻松</Button></div></div></div>}
    </article>
  </>;
}
