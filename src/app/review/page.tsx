...
function ReviewPageContent() {
  ...
  const revealStudyPoint = (index: number) => {
    const point = activeCard.answerPoints[index];
    const pointIds = new Set([point.id, ...(!point.parentId ? activeCard.answerPoints.filter((item) => item.parentId === point.id).map((item) => item.id) : [])]);
    setRevealedStudyPoints((current) => [...new Set([...current, ...activeCard.answerPoints.flatMap((item, itemIndex) => pointIds.has(item.id) ? [itemIndex] : [])])]);
  };
