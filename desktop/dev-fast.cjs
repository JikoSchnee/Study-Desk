...
async function main() {
  nextProcess = spawn(node, [next, "dev", "--hostname", "127.0.0.1", "--port", String(port)], { cwd: root, stdio: "inherit", env: { ...process.env, NODE_ENV: "development" } });
  await waitForServer();
  electronProcess = spawn(electron, ["."], { cwd: root, stdio: "inherit", env: { ...process.env, STUDY_DESK_DEV_SERVER: "1", STUDY_DESK_DEV_PORT: String(port), NODE_ENV: "development" } });
